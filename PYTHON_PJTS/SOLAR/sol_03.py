...
# Consumi
axs[0].plot(data.index, data['Consumption'], color='blue', label='Consumo')
axs[0].set_ylabel('Consumo (kW)')
axs[0].legend(loc='upper right')

# Produzione Solare
axs[1].plot(data.index, data['SolarProduction'], color='orange', label='Produzione Solare')
axs[1].set_ylabel('Produzione (kW)')
axs[1].legend(loc='upper right')

# Energia in Batteria
axs[2].plot(data.index, data['Battery'], color='green', label='Energia in Batteria')
axs[2].set_ylabel('Batteria (kWh)')
axs[2].legend(loc='upper right')

# Energia Rivenduta
axs[3].plot(data.index, data['GridExport'], color='red', label='Energia Rivenduta')
axs[3].set_ylabel('Rivendita (kW)')
axs[3].legend(loc='upper right')

# Formato per asse X (data e ora dinamica)
date_format = DateFormatter("%Y-%m-%d %H:%M")
axs[3].xaxis.set_major_formatter(date_format)

# Auto locator per asse X
locator = AutoDateLocator()
axs[3].xaxis.set_major_locator(locator)

# Aggiornamento dinamico della formattazione dell'asse temporale
plt.xticks(rotation=45)
plt.tight_layout(rect=[0, 0, 1, 0.96])  # Rettangolo per mantenere il titolo
